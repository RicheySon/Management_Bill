...
CREATE TABLE properties (
    ...
    property_kind VARCHAR(30) NOT NULL DEFAULT 'RESIDENTIAL'
...
CREATE INDEX idx_properties_kind ON properties(property_kind);
...
-- System Sequences Table (year-based: BILL / RECEIPT)
CREATE TABLE system_sequences (
    id SERIAL PRIMARY KEY,
    sequence_type VARCHAR(20) NOT NULL, -- BILL, RECEIPT (legacy PROPERTY/BUSINESS retained)
    year INTEGER NOT NULL,
    last_number INTEGER NOT NULL DEFAULT 0,
    prefix VARCHAR(10) NOT NULL, -- GN-BILL, GN-RCT, etc.
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE(sequence_type, year)
);

-- Initialize bill/receipt sequences for 2026
...
-- (Property Rate / Business Property / BOP)
CREATE TABLE area_sequences (
    id SERIAL PRIMARY KEY,
    sequence_type VARCHAR(30) NOT NULL, -- PROPERTY, BUSINESS_PROPERTY, BUSINESS
...
CREATE INDEX idx_area_sequences_lookup
...
-- First 3 letters of a place name (A–Z), padded with X if shorter
...
-- Customer code: PREFIX + EA3 + COMMUNITY3 + NNNNNN (sequence per electoral area)
...
-- Year-based auto number for bills / receipts (PREFIX-YEAR-NNNNNN)
...
-- Trigger to auto-generate property number (GNPRO / GNBPRO…)
...
-- Trigger to auto-generate business number (GNBOP…)
